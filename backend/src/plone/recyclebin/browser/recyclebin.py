...
class IRecycleBinItemForm(Interface):
    ...
    target_container = schema.TextLine(
        ...
        description=_(
            "Enter the path to the container where the item should be restored "
...
class RecycleBinWorkflowMixin:
    ...
    def _get_workflow_state(self, item):
        ...
        obj = item.get("object")
        if not obj and hasattr(self, "recycle_bin") and "recycle_id" in item:
...
class RecycleBinView(RecycleBinWorkflowMixin, form.Form):
    ...
    ignoreContext = True
    template = ViewPageTemplateFile("recyclebin.pt")
    ...
    def handle_restore(self, action):
        ...
        _data, _errors = self.extractData()
        ...
        if missing_parents_count > 0:
            if len(missing_parent_items) == 1:
                ...
                message = translate(
                    _(
                        "The item '${title}' could not be restored because its "
                ...
                message = translate(
                    _(
                        "${count} items could not be restored because their "
                        "original locations no longer exist. Please visit each item's "
                        "detail page to specify a new location.",
    ...
    def get_clear_url(self, param_to_remove):  # noqa: C901
        """Generate a URL that clears one filter while preserving the others.
    ...
    def _check_item_matches_search(self, item, search_query):
        ...
        # Search in type
        return search_query in item.get("portal_type", "").lower()
    ...
    def _check_item_matches_date_range(self, item, date_from, date_to):
        ...
        return not (date_to and item_date > date_to)
    ...
    def _find_matching_children(self, item, search_query):
        ...
        if "children" in item and isinstance(item["children"], dict):
            ...
            for _child_id, child_data in item["children"].items():
    ...
    def _apply_sorting(self, items, sort_option):  # noqa: C901
        ...
        for item in items:
            if item.get("id") not in child_items_to_exclude:
                ...
                if filter_has_subitems:
                    ...
                    if (
                        filter_has_subitems == "with_subitems" and not has_children
                    ) or (filter_has_subitems == "without_subitems" and has_children):
                        continue
...
class RecycleBinItemView(RecycleBinWorkflowMixin, form.Form):
    ...
    ignoreContext = True
    template = ViewPageTemplateFile("recyclebin_item.pt")
    item_id = None
    ...
    def handle_restore(self, action):
        ...
        if target_path:
            try:
                ...
                message = translate(
                    _(
                        "The folder '${path}' where you are trying to restore this "
                        "item cannot be found. It may have been moved or deleted. "
                        "Please choose a different location.",
        ...
        if restored_obj:
            ...
            message = translate(
                _(
                    "Failed to restore item. It may have been already restored or "
                    "deleted."
                ),
                context=self.request,
            )
    ...
    def _handle_child_restoration(self):
        ...
        if not target_path:
            ...
            if not target_path:
                message = translate(
                    _(
                        "Could not determine the original location for this child "
                        "item. "
    ...
    def get_item(self):
        ...
        if item is None:
            ...
            logger.debug(f"Found item: {title} of type {item_type}")
            # Include the total number of descendants, not only direct children.
            if "children" in item:
                item["children_count"] = self._count_descendants(item["children"])
