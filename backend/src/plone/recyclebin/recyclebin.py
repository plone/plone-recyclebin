...
ANNOTATION_KEY = "plone.recyclebin"
...
class RecycleBin:
    ...
    def add_item(
        ...
        if not IContentish.providedBy(obj):
            raise TypeError(
                f"Object {obj!r} does not provide IContentish interface. "
    ...
    def _children_match(self, children_dict, field, value, exact=False):
        ...
        for child_data in children_dict.values():
            ...
            if (
                ...
                and nested
                and self._children_match(nested, field, value, exact=exact)
            ):
                return True
        return False
    ...
    def _update_workflow_history(self, obj, action_type, item_data=None):
        ...
        Args:
            ...
            item_data: Recycle-bin data used to show the deletion date on restore
        """
    ...
    def _reset_workflow_state_if_needed(self, obj):
        ...
        if (
            ...
            and hasattr(obj, "workflow_history")
            and workflow_id in obj.workflow_history
        ):
            ...
            if history:
                ...
                reset_entry = {
                    ...
                    "actor": user_id,
                    "comments": _(
                        "Workflow state reset to '${initial_state}' during "
                ...
                logger.info(
                    "Reset workflow state of %s from '%s' to '%s'",
                    obj.getId(),
                    current_state,
                    initial_state,
                )
    ...
    def _reset_folder_children_workflow_if_needed(self, folder_obj):
        """Reset workflow states of folder children when configured."""
        settings = self._get_settings()
        if not settings.restore_to_initial_state:
            return
    ...
    def _cleanup_child_references(self, item_data):
        ...
        if "children" in item_data and isinstance(item_data["children"], dict):
            ...
            def cleanup_children(children_dict):
                for _child_id, child_data in children_dict.items():
    ...
    def _handle_existing_object(self, obj_id, target_container, obj):
        ...
        if obj_id in target_container:
            raise ValueError(
                f"Cannot restore item '{obj_id}' because an item with this ID "
                "already exists in the target location. To replace the existing "
                "item with the recycled one, use the recycle bin interface."
            )
    ...
    def purge_item(self, item_id) -> bool:
        ...
        try:
            ...
            logger.error(f"Error purging item {item_id}: {e!s}")
            return False
    ...
    def _purge_expired_items(self):
        ...
        try:
            ...
            while oldest_item := self.storage.get_oldest_item():
                ...
                if deletion_date and deletion_date < cutoff_date:
                    if self.purge_item(item_id):
                        purge_count += 1
                        logger.info(
                            "Item %s purged due to retention policy (deleted on %s)",
                            item_id,
                            deletion_date,
                        )
                    else:
            ...
            logger.error(f"Error purging expired items: {e!s}")
            return 0
