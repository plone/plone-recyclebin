...
class IRecycleBinSettings(Interface):
    ...
    restore_to_initial_state = schema.Bool(
        ...
        description=_(
            "When enabled, restored content will be set to its initial workflow "
            "state (usually 'draft') instead of the workflow state it was in when "
            "deleted."
        ),
        default=False,
        required=False,
...
class IRecycleBin(Interface):
    ...
    def search(
        ...
        Args:
            ...
            has_subitems: True for items with children; False for items without
